    ...
    @if(auth()->user()->isCustomer())
        ...
        @if($promoProducts->isNotEmpty())
            @if(!$isClosed && !$isMinimized)
                ...
                <div id="promo-widget-overlay"
                    class="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center p-4 promo-widget-overlay-container backdrop-blur-sm">
                    <div class="bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-w-lg promo-widget-modal-content relative"
                        onclick="event.stopPropagation();">
                        <!-- Close Button (Top Right) -->
                        <button id="promo-widget-close"
                            class="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors focus-ring"
                            aria-label="Tutup">
                        ...
                        <div class="py-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
                            @foreach($promoProducts as $index => $product)
                                <a href="{{ route('products.show', $product) }}" class="block promo-product-link group promo-product-item">
                                    <div
                                        class="px-6 py-4 hover:bg-gray-50 transition-all duration-200 border-b border-gray-100 last:border-b-0 group-hover:shadow-sm">
                                        <div class="flex items-center justify-between gap-4">
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center gap-2 mb-1">
                                                    <p class="text-xs uppercase tracking-[0.1em] text-gray-400">
                                                        {{ $product->category->name }}
                                                    </p>
                                                    @if($product->discount_percentage > 0)
                                                        <span
                                                            class="text-xs font-bold text-red-500 bg-red-50 px-2 py-0.5 rounded promo-discount-badge">
                                                            -{{ number_format($product->discount_percentage, 0) }}%
                                                        </span>
                                                    @endif
                                                </div>
                                                <p class="text-sm font-semibold text-gray-900 mb-1.5 truncate group-hover:text-gray-700">
                                                    {{ $product->name }}
                                                </p>
            ...
            @if($isMinimized && !$isClosed)
                ...
                <div id="promo-widget-mini" class="fixed bottom-6 right-28 z-[9998]" style="z-index: 9998;">
                    <button id="promo-widget-mini-toggle"
                        ...
                        aria-label="Buka Promo" x-data="{
                                                            init() {
                                                                try {
                                                                    const widget = document.getElementById('promo-widget-mini');
                                                                    const posStr = localStorage.getItem('promo_widget_pos');
                                                                    if (posStr) {
                                                                    ...
                                                                    console.error('Error loading promo widget position:', e);
                                                                }
                                                            }
                                                        }" @mousedown="(e) => {
                                                            const widget = document.getElementById('promo-widget-mini');
                                                            ...
                                                            const onMouseUp = () => {
                                                                if (isDragging) {
                                                                    localStorage.setItem('promo_widget_pos', JSON.stringify({
                        ...
                        <span
                            class="absolute -top-1 -right-1 w-5 h-5 bg-white text-red-500 text-[10px] font-bold rounded-full flex items-center justify-center shadow-md">
                            {{ $promoProducts->count() }}
                        </span>
        ...
        @if($promoProducts->isNotEmpty())
            <style>
                ...
                .promo-product-item:nth-child(1) {
                    animation-delay: 0.1s;
                }

                .promo-product-item:nth-child(2) {
                    animation-delay: 0.15s;
                }

                .promo-product-item:nth-child(3) {
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    if (miniToggle) {
                        miniToggle.addEventListener('click', function (e) {
                            if (miniWidget.dataset.justDragged) {
                                e.preventDefault();
                                e.stopPropagation();
                                return;
                            }
