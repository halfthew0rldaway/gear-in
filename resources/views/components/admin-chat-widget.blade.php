@auth
    @if(auth()->user()->isAdmin())
        <div id="admin-chat-widget" class="fixed bottom-6 right-6 z-50">
            <!-- Chat Button (Floating) -->
            <button 
                id="admin-chat-widget-toggle" 
            ...
            >
                ...
                @if($unreadConversations > 0)
                    <span class="absolute -top-1 -right-1 w-6 h-6 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center animate-pulse">
            ...
            >
                    ...
                    <div id="admin-chat-widget-conversations" class="p-4 space-y-2">
                        ...
                        @if($recentConversations->count() > 0)
                            @foreach($recentConversations as $conv)
                                <button 
                                    onclick="loadAdminConversation({{ $conv->id }})"
                                ...
                                >
                                    <div class="flex items-center justify-between">
                                        ...
                                        @if($conv->unreadMessagesCount() > 0)
                                            <span class="px-2 py-1 text-xs bg-red-500 text-white rounded-full">{{ $conv->unreadMessagesCount() }}</span>
                                        @endif
                                    </div>
                                    @if($conv->latestMessage)
                                        <p class="text-xs text-gray-600 line-clamp-1">{{ Str::limit($conv->latestMessage->body, 50) }}</p>
                                    @endif
                                    <p class="text-xs text-gray-400">{{ $conv->last_message_at ? $conv->last_message_at->diffForHumans() : '' }}</p>
                                </button>
                            @endforeach
                            <a href="{{ route('admin.chat.index') }}" class="block w-full p-3 text-center text-sm text-gray-600 hover:text-gray-900">
                ...
                <div id="admin-chat-widget-input-container" class="hidden border-t border-gray-200 p-4 bg-white">
                    <form id="admin-chat-widget-form" class="flex gap-2">
                        ...
                        <textarea 
                            name="body" 
                            id="admin-chat-widget-input" 
                            rows="2" 
                            required 
                            placeholder="Tulis balasan..." 
                            class="flex-1 rounded-2xl border border-gray-300 px-4 py-3 text-sm text-gray-900 focus:border-gray-900 focus:ring-gray-900 resize-none"
                        ></textarea>
                        <button 
                            type="submit" 
