        ...
        <form action="{{ route('catalog') }}" method="GET" class="space-y-4">
            <div class="bg-white border border-gray-200 rounded-3xl p-5 space-y-5">
                    ...
                    <div class="flex items-center gap-4">
                        <label for="sort-select" class="flex items-center gap-2 text-sm text-gray-600">
                            ...
                            <select name="sort" id="sort-select" onchange="this.form.submit()"
                                class="rounded-lg border-gray-300 focus:border-gray-900 focus:ring-gray-900 text-sm">
                                <option value="newest" {{ ($sortBy ?? 'newest') === 'newest' ? 'selected' : '' }}>Terbaru
                                </option>
                                <option value="price_low" {{ ($sortBy ?? '') === 'price_low' ? 'selected' : '' }}>Harga:
                                    Rendah ke Tinggi</option>
                                <option value="price_high" {{ ($sortBy ?? '') === 'price_high' ? 'selected' : '' }}>Harga:
                                    Tinggi ke Rendah</option>
                                <option value="name_asc" {{ ($sortBy ?? '') === 'name_asc' ? 'selected' : '' }}>Nama: A-Z
                                </option>
                                <option value="name_desc" {{ ($sortBy ?? '') === 'name_desc' ? 'selected' : '' }}>Nama: Z-A
                                </option>
                                <option value="rating" {{ ($sortBy ?? '') === 'rating' ? 'selected' : '' }}>Rating Tertinggi
                                </option>
                        ...
                        <a href="{{ route('catalog') }}"
                            class="text-xs uppercase tracking-[0.5em] text-gray-500 hover:text-gray-900">Atur ulang</a>
                    </div>
                </div>
                ...
                <div class="border-t border-gray-200 pt-5 space-y-4">
                    <div class="relative">
                        <div
                            class="flex items-center gap-3 border border-gray-300 rounded-full px-4 py-2 bg-white focus-within:border-gray-900 focus-within:ring-1 focus-within:ring-gray-900">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-500" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor">
                                <circle cx="11" cy="11" r="7" />
                                <line x1="16.65" y1="16.65" x2="21" y2="21" />
                            </svg>
                            <input type="search" name="q" value="{{ old('q', $searchQuery) }}"
                    ...
                    <div class="flex flex-wrap gap-3">
                        <a href="{{ route('catalog', $queryString(['category' => null])) }}"
                        ...
                        @foreach ($categories as $category)
                            ...
                            @endphp
                            <a href="{{ route('catalog', $queryString(['category' => $category->slug])) }}"
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 pt-4 border-t border-gray-200">
                        ...
                        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.4em] text-gray-500">
                            <input type="checkbox" name="out_of_stock" value="1" {{ $filters['out_of_stock'] ? 'checked' : '' }} class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                            Stok Habis
                        </label>
                        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.4em] text-gray-500">
                    ...
                    <div class="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                        <button type="submit"
                            class="px-5 py-2 rounded-full bg-gray-900 text-white text-xs uppercase tracking-[0.4em]">Terapkan
                            Filter</button>
                        <a href="{{ route('catalog') }}"
                            class="px-5 py-2 rounded-full border border-gray-300 text-gray-600 text-xs uppercase tracking-[0.4em] hover:border-gray-900 hover:text-gray-900">Atur
                            Ulang</a>
                    </div>
                </div>
            </div>
