...
<head>
    ...
    <title>Laporan Dashboard & Pendapatan - gear-in</title>
    <style>
        @media print {
            body {
                margin: 0;
...
<body>
    <div class="no-print">
        ...
        <a href="{{ route('admin.dashboard') }}"
            style="display: inline-block; padding: 12px 24px; background: #6f6f6f; color: white; text-decoration: none; border-radius: 6px; margin-left: 10px;">←
            Kembali ke Dashboard</a>
    </div>

    <div class="header">
        <h1>gear-in</h1>
        <p>Laporan Dashboard & Pendapatan</p>
        <p>Ringkasan Statistik & Performa Penjualan</p>
    </div>
    ...
    <div class="section">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-value revenue-highlight">{{ 'Rp ' . number_format($stats['revenue'], 0, ',', '.') }}
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Pendapatan Bulan Ini</div>
                <div class="stat-value revenue-highlight">
                    {{ 'Rp ' . number_format($stats['revenue_this_month'], 0, ',', '.') }}</div>
            </div>
            <div class="stat-card">
    ...
    <div class="section">
        ...
        <div class="two-column">
            ...
            <div class="summary-box">
                <h3>Pendapatan per Status</h3>
                <div class="summary-item">
                    <span>Selesai (Completed)</span>
                    <span
                        class="revenue-highlight">{{ 'Rp ' . number_format($revenueByStatus['completed'], 0, ',', '.') }}</span>
                </div>
                <div class="summary-item">
                    <span>Dikirim (Shipped)</span>
                    <span
                        class="revenue-highlight">{{ 'Rp ' . number_format($revenueByStatus['shipped'], 0, ',', '.') }}</span>
                </div>
                <div class="summary-item">
                    <span>Dibayar (Paid)</span>
                    <span
                        class="revenue-highlight">{{ 'Rp ' . number_format($revenueByStatus['paid'], 0, ',', '.') }}</span>
                </div>
                <div class="summary-item">
                    <span>Menunggu (Pending)</span>
                    <span
                        class="revenue-highlight">{{ 'Rp ' . number_format($revenueByStatus['pending'], 0, ',', '.') }}</span>
                </div>
                <div class="summary-item">
        ...
        <table class="data-table">
            ...
            <tbody>
                @foreach($dailyRevenue as $day)
                    <tr>
                        <td>{{ $day['date'] }}</td>
                        <td class="text-right revenue-highlight">{{ 'Rp ' . number_format($day['revenue'], 0, ',', '.') }}
                        </td>
                ...
                <tr style="background: #f9f9f9; font-weight: 600;">
                    <td>Total</td>
                    <td class="text-right revenue-highlight">
                        {{ 'Rp ' . number_format(collect($dailyRevenue)->sum('revenue'), 0, ',', '.') }}</td>
        ...
        <table class="data-table">
            ...
            <tbody>
                @foreach($monthlyRevenue as $month)
                    <tr>
                        <td>{{ $month['month'] }}</td>
                        <td class="text-right revenue-highlight">{{ 'Rp ' . number_format($month['revenue'], 0, ',', '.') }}
                        </td>
                ...
                <tr style="background: #f9f9f9; font-weight: 600;">
                    <td>Total</td>
                    <td class="text-right revenue-highlight">
                        {{ 'Rp ' . number_format(collect($monthlyRevenue)->sum('revenue'), 0, ',', '.') }}</td>
    ...
    @if($topProducts->count() > 0)
        <div class="section">
            <div class="section-title">Top 10 Produk Terlaris (Berdasarkan Pendapatan)</div>
            <table class="data-table">
                <thead>
                    <tr>
                        <th>No</th>
                ...
                <tbody>
                    @foreach($topProducts as $index => $product)
                        <tr>
                            ...
                            <td class="text-right revenue-highlight">
                                {{ 'Rp ' . number_format($product->total_revenue, 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                    <tr style="background: #f9f9f9; font-weight: 600;">
                        ...
                        <td class="text-right revenue-highlight">
                            {{ 'Rp ' . number_format($topProducts->sum('total_revenue'), 0, ',', '.') }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div
        style="margin-top: 40px; padding-top: 20px; border-top: 2px solid #dfdfdf; text-align: center; font-size: 10px; color: #6f6f6f;">
