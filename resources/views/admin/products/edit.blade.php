    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="bg-white border border-gray-200 rounded-[32px] p-6 space-y-4">
        ...
        <div class="border-t border-gray-200 pt-6 mt-6">
            ...
            <div class="grid sm:grid-cols-3 gap-4">
                <label class="text-xs uppercase tracking-[0.4em] text-gray-400 block">
                    Persentase Diskon (%)
                    <input type="number" name="discount_percentage" value="{{ old('discount_percentage', $product->discount_percentage) }}" min="0" max="100" step="0.01" class="mt-2 w-full rounded-2xl border border-gray-200 px-4 py-3 focus:border-gray-900 focus:ring-gray-900 focus-ring">
                    <p class="text-xs text-gray-500 mt-1">Kosongkan jika tidak ada diskon</p>
                </label>
                <label class="text-xs uppercase tracking-[0.4em] text-gray-400 block">
                    Mulai Diskon
                    <input type="datetime-local" name="discount_starts_at" value="{{ old('discount_starts_at', $product->discount_starts_at ? $product->discount_starts_at->format('Y-m-d\TH:i') : '') }}" class="mt-2 w-full rounded-2xl border border-gray-200 px-4 py-3 focus:border-gray-900 focus:ring-gray-900 focus-ring">
                </label>
                <label class="text-xs uppercase tracking-[0.4em] text-gray-400 block">
                    Berakhir Diskon
                    <input type="datetime-local" name="discount_expires_at" value="{{ old('discount_expires_at', $product->discount_expires_at ? $product->discount_expires_at->format('Y-m-d\TH:i') : '') }}" class="mt-2 w-full rounded-2xl border border-gray-200 px-4 py-3 focus:border-gray-900 focus:ring-gray-900 focus-ring">
                </label>
            </div>
        </div>
        <label class="text-xs uppercase tracking-[0.4em] text-gray-400 block">
